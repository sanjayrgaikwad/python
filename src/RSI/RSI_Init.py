...
while (i < len(tickers)) and (Amount_of_API_Calls < 1800):
    try:
        ...
        Hist_data = temp.history(period="4mo")  # Tells yfinance what kind of data we want about this stock (In this example, all of the historical data)
        #dayOfweek = 'W-' + datetime.datetime.now().strftime('%a').upper() #to get data weekly ,can use this if using not on friday 
        #Hist_data = Hist_data.asfreq(dayOfweek, method='pad') #'W-FRI' instread of dayOfweek 
        Hist_data.to_csv("B:\Sheru\coding\python\python-rsi-master\src\RSI\data\\"+stock +".csv")  # Saves the historical data in csv format for further processing later
